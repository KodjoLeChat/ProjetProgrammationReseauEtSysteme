...
class Adding_Building(SelectionneurZone):
    ...
    def treat_event(self, grid, last_grid):
        if (grid[0] >= 0 and grid[1] >= 0 and len(self.carriere.informations_tiles) > grid[0] and len(
                self.carriere.informations_tiles[grid[0]]) > grid[1] and
                self.carriere.controleur.check_if_construction_possible_on_grid(grid) and grid != (
                20, 39) and self.carriere.controleur.check_if_path_exist_from_spawn_walker(grid)):
            ...
            for num_lig in range(0, len(self.carriere.informations_tiles)):
                for num_col in range(0, len(self.carriere.informations_tiles[num_lig])):
                    if stop_loop == False and self.carriere.informations_tiles[num_lig][num_col]["building"].name[
                                              0:5] == "route" and self.calcul_distance_to_grid(grid,
                                                                                               (num_lig, num_col)) <= 2:
                        ...
                        self.carriere.controleur.add_building_on_point(grid, self.carriere.dictionnaire_reverse_by_path[
                            self.path])
    ...
    def treat_event_local(self, grid, last_grid):
        if (grid[0] >= 0 and grid[1] >= 0 and len(self.carriere.informations_tiles) > grid[0] and len(
                self.carriere.informations_tiles[grid[0]]) > grid[1] and
    ...
    def treat_event_local_old(self, grid, last_grid):
