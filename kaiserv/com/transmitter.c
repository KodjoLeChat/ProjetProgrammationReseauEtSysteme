...
void print_ip_list(struct ip_list* list) {
    printf("Liste des adresses IP connectées :\n");
    struct ip_node* current = list->head;
    while (current != NULL) {
        char ip[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &(current->addr.sin_addr), ip, INET_ADDRSTRLEN);
        printf("%s:%d\n", ip, ntohs(current->addr.sin_port));
        current = current->next;
    }
}
        ...
        if (FD_ISSET(python_client_socket, &readfds)) {
            if ((valread = read(python_client_socket, buffer, sizeof(buffer))) == 0) {
                ...
                if (udpServerSocket > 0) {
                    ...
                    char message[MAX_BUFFER]="broadcast\n"; 

                    
                    sendto(udpServerSocket, message, strlen(message), 0, (struct sockaddr*)&broadcast_address, sizeof(broadcast_address));
                    ...
                    while (1) {
                        ...
                        while (current != NULL) {
                            ...
                            print_ip_list(&ip_list_connected);
