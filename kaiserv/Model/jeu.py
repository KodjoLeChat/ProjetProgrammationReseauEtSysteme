...
class Jeu:
    ...
    def update(self):
        ...
        for walker in self.walkerlist:
            ...
            if walker.name == "citizen":  # citizen => migrant qui viennent vers leur future maison
                ...
                if walker.actualPosition == walker.destination and \
                        self.monde.board[walker.destination[0]][walker.destination[1]]["building"].name == 'panneau':
                    self.monde.board[walker.destination[0]][walker.destination[1]][
                        "building"] = self.monde.craft_building(self.monde.information_for_each_tile['tente'],
                                                                self.ressources)
                    self.monde.board[walker.destination[0]][walker.destination[1]]["building"].set_position_reference(
                        walker.destination)
                    should_refresh = True
                        ...
                        if walker.name == "citizen_engeneer" and position_reference == walker.position_reference:
                            ...
                            for coord in matrix_for_look_around_road:
                                ...
                                if new_grid[0] >= 0 and new_grid[1] >= 0 and \
                                        new_grid[0] < len(self.monde.board) and new_grid[1] < len(
                                    self.monde.board[new_grid[0]]) and \
                                        self.monde.board[new_grid[0]][new_grid[1]]["building"].name[
                                        0:5] == "route" and self.is_chemin_with_road(walker.actualPosition, new_grid):
    ...
    def walker_creation(self, depart, destination):
        if self.ressources.enough_dinars(1000):
            ...
            walker.chemin = short_path(numpy.array(self.monde.define_matrix_for_path_finding()), walker.actualPosition,
                                       walker.destination)
            walker.set_nextPosition()