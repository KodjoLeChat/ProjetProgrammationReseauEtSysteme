...
class Road:
    ...
    def get_image_key(self, coord):
        ...
        if (x, y - 1) in self.worldModel.get_list_grid_pos_road():
            type["right"] = True
        # ligne top bottom
        if type["top"] and type["bottom"] and not (type["right"] or type["left"]):
            return "top_bottom"
        # ligne left right
        if type["right"] and type["left"] and not (type["top"] or type["bottom"]):
            return "right_left"
        # turn_right_to_bottom
        if type["right"] and type["bottom"] and not (type["top"] or type["left"]):
            return "turn_right_to_bottom"
        # turn_bottom_to_left
        if type["bottom"] and type["left"] and not (type["top"] or type["right"]):
            return "turn_bottom_to_left"
        # turn_left_to_top
        if type["left"] and type["top"] and not (type["bottom"] or type["right"]):
            return "turn_left_to_top"
        # turn_top_to_right
        if type["top"] and type["right"] and not (type["bottom"] or type["left"]):
        ...
        if type["top"] and type["right"] and type["bottom"] and type["left"]:
            return "cross"
        # top_end
        if type["top"] and not (type["right"] or type["bottom"] or type["left"]):
            return "top_end"
        # bottom_end
        if type["bottom"] and not (type["top"] or type["right"] or type["left"]):
            return "bottom_end"
        # right_end
        if type["right"] and not (type["top"] or type["bottom"] or type["left"]):
            return "right_end"
        # left_end
        if type["left"] and not (type["top"] or type["right"] or type["bottom"]):
            return "left_end"
        else :
            return "right_left"
